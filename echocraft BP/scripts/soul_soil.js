...
world.afterEvents.playerBreakBlock.subscribe((event) => {
    ...
    if (brokenBlock.type.id !== ("minecraft:soul_soil")) {
        return;
    }

    const itemEnchantComp = item.getComponent("minecraft:enchantable");

    if (brokenBlock.type.id === "minecraft:soul_soil" && !itemEnchantComp?.hasEnchantment("silk_touch") && item.typeId.startsWith("echocraft") && item.typeId.endsWith("echo_shovel")) {
